...
document.querySelector('button').addEventListener('click', function () {
  ...
  for (const [i, variable] of variables.entries()) {
    ...
    for (const [j, w] of words.entries()) {
      j == 0
        ? camelCase.push(w)
        : camelCase.push(w.replace(w[0], w[0].toUpperCase()));
    }
    console.log(`${camelCase.join('').padEnd(20)}${'✅'.repeat(i + 1)}`);
  }
});
*/
const flights =
...
for (const flight of flights.split('+')) {
  console.log(makeReadableFlight(flight));
}

function makeReadableFlight(flight) {
  const [type, from, to, time] = flight.split(';');
  return `${formatType(type)} from ${formatAirport(from)} to ${formatAirport(
    to
  )} (${formatTime(time)})`.padStart(44);
}

function formatType(type) {
  let result = type.slice(1).replace('_', ' ');
  result = result.startsWith('Delayed') ? `🔴 ${result}` : result;
  return result;
}
